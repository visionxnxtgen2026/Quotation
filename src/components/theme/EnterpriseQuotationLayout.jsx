...
/**
 * 🏛️ EnterpriseQuotationLayout — Master Premium Corporate Quotation Component
 * Designed to Oracle ERP, SAP, Zoho Books & Tally Prime enterprise presentation standards.
 * Features strict grid alignment, typography hierarchy, vertical centering, and balanced spacing.
 */
export default function EnterpriseQuotationLayout({ data }) {
  ...
  return (
    <div className="bg-white text-slate-900 font-sans p-8 sm:p-12 min-h-[297mm] w-full max-w-[794px] mx-auto relative leading-relaxed text-xs box-border">
      
      {/* 🏢 1. CORPORATE HEADER */}
      <div className="border-b-2 border-slate-900 pb-6 mb-6 flex justify-between items-center gap-8">
        {/* Company Identity */}
        <div className="flex items-center gap-6 flex-1 min-w-0">
          ...
          {hasCompanyHeader && (
            <div className="flex-1 min-w-0 space-y-1 text-left">
              ...
              {hasAddress && (
                <p className="text-[11px] text-slate-600 font-medium whitespace-pre-line leading-tight">{quote.companyAddress}</p>
              )}
              {(hasPhone || hasEmail) && (
        ...
        {/* Document Title & Reference Header (Right Aligned) */}
        <div className="text-right shrink-0 flex flex-col items-end justify-center space-y-1.5">
          ...
          {hasDocHeader && (
            <div className="text-[11px] space-y-1 text-slate-700 font-medium text-right">
              {hasRefNo && (
            ...
            <div className="space-y-1.5 flex-1 text-left">
              ...
              {hasWarranty && (
                <div className="pt-1.5">
                  <span className="inline-flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 border border-blue-200/80 text-[10px] font-extrabold leading-none whitespace-nowrap">
          ...
          {validSections.map((sec, secIdx) => {
            ...
            return (
              <div key={sec.id || secIdx} className="border border-slate-200/90 rounded-xl overflow-hidden shadow-2xs">
                ...
                {/* Items Table with Fixed Layout & Explicit Alignment */}
                    ...
                    {secItems.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.id || idx} className="hover:bg-slate-50/50">
                          <td className="py-3 px-3 border-r border-slate-200/80 text-center font-mono text-[11px] text-slate-500 align-middle">{idx + 1}</td>
                          <td className="py-3 px-3 border-r border-slate-200/80 font-medium whitespace-pre-wrap leading-relaxed align-middle text-left">{item.desc || item.work || item.description || "—"}</td>
                          {showLabour && <td className="py-3 px-3 border-r border-slate-200/80 text-right font-mono font-medium text-slate-700 align-middle">₹{lab.toFixed(2)}</td>}
                          {showMaterial && <td className="py-3 px-3 border-r border-slate-200/80 text-right font-mono font-medium text-slate-700 align-middle">₹{mat.toFixed(2)}</td>}
                          <td className="py-3 px-3 text-right font-mono font-bold text-slate-900 align-middle">₹{tot.toFixed(2)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                <div className="bg-slate-900 text-white px-4 py-2.5 flex items-center justify-between font-bold text-xs">
                  <span className="uppercase text-[11px] tracking-wide text-slate-300">Category Rate Summary</span>
                  <div className="flex items-center gap-4 text-xs">
                    ...
                    <span className="text-emerald-400 font-black text-sm font-mono">Rate/Sqft: ₹{secRatePerSqft.toFixed(2)}</span>
                  </div>
                </div>

                {/* Working Area Calculation Panel */}
                {workingAreaNum > 0 && (
                  <div className="bg-slate-50 px-4 py-2.5 border-t border-slate-200 flex items-center justify-between text-xs font-semibold text-slate-700">
                    <span>Calculation: {workingAreaNum} Sqft × ₹{secRatePerSqft.toFixed(2)}/Sqft</span>
                    <span className="font-black text-slate-900 text-sm font-mono">Category Total: ₹{secEstimatedAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                )}
              </div>
            );
        ...
        <div className="flex justify-end mb-6 break-inside-avoid print:break-inside-avoid">
          <div className="w-80 border border-slate-300 rounded-xl overflow-hidden bg-slate-50/50 shadow-2xs">
            ...
            <div className="p-4 bg-slate-900 text-white flex justify-between items-center">
              ...
              <span className="font-mono text-lg font-black text-emerald-400 text-right">
      ...
      {/* 📝 5. SCOPE OF WORK, TERMS & EXCLUSIONS */}
        ...
        <div className="space-y-5 mb-6">
          ...
          {hasBankDetails && (
            <div className="border border-slate-200 rounded-xl p-5 bg-slate-50/80 shadow-2xs break-inside-avoid">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-[11px] font-semibold text-slate-700">
          ...
          {hasTerms && (
            <div className="border border-slate-200 rounded-xl p-5 bg-white shadow-2xs">
              ...
              <ol className="list-decimal list-inside space-y-1.5 text-slate-700 font-medium leading-relaxed text-xs">
      ...
      <div className="pt-6 mt-6 border-t-2 border-slate-900 flex justify-between items-end break-inside-avoid print:break-inside-avoid">
        ...
        <div className="text-[11px] text-slate-500 font-medium space-y-1 flex-1 pr-6 text-left">
          ...
          {hasWebsite && <p className="text-slate-700 font-extrabold mt-1.5">{quote.website}</p>}
        </div>
