...
/**
 * GROUPED — Structured Proposal Enterprise Quotation (Rose Accent, Executive Proposal Style)
 */
export default function GroupedTemplate({ data }) {
  ...
  return (
    <div className="bg-white p-12 min-h-[297mm] text-slate-800 font-sans border-t-8 border-rose-600 border-x border-b border-slate-200 relative text-xs leading-relaxed shadow-xs">
      
      {/* 🏢 1. HEADER */}
      <div className="flex justify-between items-start pb-8 border-b border-slate-200 mb-8 gap-8">
        <div className="flex gap-5 items-start">
          {hasLogo && (
            <img src={quote.companyLogo} alt="Logo" className="w-16 h-16 object-contain shrink-0 rounded-xl border border-slate-200 p-1" />
          )}
          {hasCompanyHeader && (
            <div>
              {hasCompanyName && (
              ...
              {hasTagline && <p className="text-[11px] font-bold text-rose-600 mb-1">{quote.companyTagline}</p>}
        ...
        <div className="text-right shrink-0">
          <span className="bg-rose-50 text-rose-700 text-xs font-black px-3.5 py-1.5 rounded-full uppercase tracking-widest inline-block mb-3 border border-rose-200">
            Structured Proposal
          </span>
          {hasDocHeader && (
              ...
              <tbody>
                {hasRefNo && (
                  <tr>
                    ...
                    <td className="font-mono font-bold text-rose-950 py-0.5">{quote.quotationNo}</td>
                  </tr>
                )}
                {hasDate && (
                  <tr>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          {hasClientSection && (
            <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-rose-600 mb-2">
                Client Details
              </h3>
          ...
          {hasProjectSection && (
            <div className="bg-rose-50/40 p-5 rounded-xl border border-rose-100">
              ...
              {hasWarranty && (
                <div className="mt-3 inline-flex items-center gap-1.5 bg-rose-600 text-white text-[10px] font-bold px-3 py-1 rounded-lg uppercase tracking-wider shadow-xs">
      ...
      {validSections.length > 0 && (
        <div className="mb-8 space-y-6">
          {validSections.map((sec, secIdx) => (
            <div key={secIdx}>
              {hasVal(sec.title) && (
                <div className="bg-slate-900 text-white font-bold text-xs uppercase px-4 py-2 rounded-t-xl tracking-wider">
                  {sec.title}
                </div>
              )}
              <div className="border border-slate-200 overflow-hidden rounded-b-xl shadow-xs">
      ...
      {hasGrandTotal && (
        <div className="flex justify-end mb-8">
          <div className="w-80 rounded-xl border border-slate-200 overflow-hidden shadow-xs bg-slate-50">
            ...
            <div className="p-4 bg-rose-600 text-white flex justify-between items-center font-bold text-sm">
          ...
          <div>
            {hasBankDetails && (
              <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/80">
                <h4 className="font-bold text-rose-900 uppercase tracking-wider text-[11px] mb-3 pb-1 border-b border-slate-200">
      ...
      <div className="pt-8 border-t border-slate-200 flex justify-between items-end">
        <div className="text-[11px] text-slate-500">
          <p className="font-semibold text-slate-800">Thank you for choosing {quote.companyName || "us"}!</p>
          {hasWebsite && <p className="mt-0.5 text-rose-600">{quote.website}</p>}
        </div>
