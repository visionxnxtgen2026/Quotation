...
/**
 * SIGNATURE — Premium Luxury Corporate Quotation (High-End Client Presentation, Gold / Slate Luxury Accents)
 */
export default function SignatureTemplate({ data }) {
  ...
  return (
    <div className="bg-white p-12 min-h-[297mm] text-slate-900 font-sans border border-slate-200 relative text-xs leading-relaxed shadow-sm">
      
      {/* SIGNATURE LUXURY HEADER */}
      <div className="flex justify-between items-start pb-8 border-b-2 border-amber-500 mb-8 gap-8">
        <div className="flex gap-5 items-start">
          {hasLogo && (
            <img src={quote.companyLogo} alt="Logo" className="w-18 h-18 object-contain shrink-0 rounded-2xl border border-amber-200 p-1 bg-amber-50/20" />
          )}
          {hasCompanyHeader && (
            <div>
              {hasCompanyName && (
              ...
              {hasTagline && <p className="text-[11px] font-bold text-amber-600 mb-1">{quote.companyTagline}</p>}
              <div className="text-[11px] text-slate-500 space-y-0.5 mt-1 font-medium">
                ...
                {hasGst && <p className="text-amber-700 font-semibold">GSTIN: {quote.gstNo}</p>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="text-right shrink-0">
          <span className="bg-slate-900 text-amber-400 text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest inline-block mb-3 border border-amber-500/30">
            Signature Proposal
          </span>
          {hasDocHeader && (
              ...
              <tbody>
                {hasRefNo && (
                  <tr>
                    ...
                    <td className="font-mono font-bold text-slate-900 py-0.5">{quote.quotationNo}</td>
                  </tr>
                )}
                {hasDate && (
                  <tr>
      ...
      {/* LUXURY METADATA BLOCK */}
      {(hasClientSection || hasProjectSection) && (
        <div className="grid grid-cols-2 gap-8 mb-8">
          {hasClientSection && (
            <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-2">
          ...
          {hasProjectSection && (
            <div className="bg-amber-50/30 p-5 rounded-2xl border border-amber-200">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-2">
              ...
              {hasBrand && <p className="text-slate-600 mt-1 font-semibold">Brand Standard: {quote.paintBrand}</p>}
              {hasWarranty && (
                <div className="mt-3 inline-flex items-center gap-1.5 bg-amber-500 text-slate-950 text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-wider shadow-xs">
                  <span>✦ Signature {quote.warranty} Year Guarantee</span>
                </div>
              )}
            </div>
          )}
      ...
      {validSections.length > 0 && (
        <div className="mb-8 space-y-6">
          {validSections.map((sec, secIdx) => (
            <div key={secIdx}>
              {hasVal(sec.title) && (
                <h4 className="font-bold text-slate-900 uppercase tracking-wider text-xs mb-3 pb-1 border-b-2 border-amber-500">
                  {sec.title}
                </h4>
              )}
              <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-900 text-amber-400 font-bold text-[10px] uppercase tracking-wider">
      ...
      {hasGrandTotal && (
        <div className="flex justify-end mb-8">
          <div className="w-80 rounded-2xl border border-slate-200 overflow-hidden shadow-xs bg-slate-50">
            <div className="p-4 space-y-2.5 text-xs border-b border-slate-200">
              ...
              {hasDiscount && (
                <div className="flex justify-between text-emerald-600 font-semibold">
                  <span>Signature Privilege Discount</span>
            ...
            <div className="p-4 bg-slate-900 text-white flex justify-between items-center font-bold text-sm">
              <span className="uppercase tracking-wider text-xs text-amber-400">Total Proposal Value</span>
              <span className="font-mono text-lg text-amber-400">₹ {quote.grandTotal}</span>
            </div>
          </div>
        </div>
      )}
          ...
          <div>
            {hasBankDetails && (
              <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50">
                <h4 className="font-bold text-amber-700 uppercase tracking-wider text-[11px] mb-3 pb-1 border-b border-slate-200">
                ...
                <table className="text-[11px] w-full text-slate-700">
                  <tbody>
                    ...
                    {hasVal(bank.accHolder) && <tr><td className="font-semibold py-1">Account Holder:</td><td className="text-right py-1 font-medium">{bank.accHolder}</td></tr>}
                    ...
                    {hasVal(bank.ifsc) && <tr><td className="font-semibold py-1">IFSC Code:</td><td className="text-right py-1 font-mono font-semibold">{bank.ifsc}</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
      ...
      <div className="pt-8 border-t border-slate-200 flex justify-between items-end">
        <div className="text-[11px] text-slate-500">
          ...
          {hasWebsite && <p className="mt-0.5 text-amber-600">{quote.website}</p>}
        </div>

        {hasSignature && (
          <div className="text-center w-48">
            <div className="h-12 border-b border-slate-300 mb-2 flex items-end justify-center">
              <span className="text-[9px] text-slate-400 uppercase tracking-widest pb-1">[ Seal &amp; Signature ]</span>
            </div>
