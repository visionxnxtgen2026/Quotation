...
/**
 * EXECUTIVE — Large Corporate IT & Consulting Quotation (Blue & Grey Accents, Executive Summary)
 */
export default function ExecutiveTemplate({ data }) {
  ...
  return (
    <div className="bg-white p-12 min-h-[297mm] text-slate-800 font-sans border-t-8 border-blue-800 border-x border-b border-slate-200 relative text-xs leading-relaxed shadow-xs">
      
      {/* 🏢 EXECUTIVE HEADER & METADATA */}
      <div className="flex justify-between items-start pb-8 border-b-2 border-slate-200 mb-8 gap-8">
        <div className="flex gap-5 items-start">
          {hasLogo && (
            <img src={quote.companyLogo} alt="Logo" className="w-18 h-18 object-contain shrink-0 rounded-lg border border-slate-200 p-1 bg-slate-50" />
          )}
          {hasCompanyHeader && (
            <div>
              {hasCompanyName && (
                <h1 className="text-2xl font-black text-blue-900 tracking-tight leading-none mb-1">
                  {quote.companyName}
                </h1>
              )}
              {hasTagline && <p className="text-[11px] font-semibold text-slate-500 mb-1.5">{quote.companyTagline}</p>}
              <div className="text-[11px] text-slate-600 space-y-0.5 mt-1 font-medium">
                ...
                {hasGst && <p className="text-blue-900 font-bold">GSTIN: {quote.gstNo}</p>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="text-right shrink-0">
          <div className="bg-blue-900 text-white text-xs font-black px-4 py-1.5 uppercase tracking-widest inline-block mb-3 rounded-md">
            EXECUTIVE QUOTATION
          </div>
          {hasDocHeader && (
            <table className="text-[11px] text-right ml-auto text-slate-700">
              <tbody>
                {hasRefNo && (
                  <tr>
                    <td className="font-bold text-slate-400 uppercase tracking-wider pr-3 py-0.5">Reference:</td>
                    <td className="font-mono font-bold text-blue-950 py-0.5">{quote.quotationNo}</td>
                  </tr>
                )}
                {hasDate && (
                  <tr>
                    <td className="font-bold text-slate-400 uppercase tracking-wider pr-3 py-0.5">Issue Date:</td>
                    <td className="font-semibold text-slate-900 py-0.5">{quote.date}</td>
                  </tr>
                )}
                {hasExpiry && (
                  <tr>
                    <td className="font-bold text-slate-400 uppercase tracking-wider pr-3 py-0.5">Expiry Date:</td>
                    <td className="font-semibold text-slate-900 py-0.5">{quote.expiryDate}</td>
                  </tr>
                )}
                {hasRevision && (
                  <tr>
                    <td className="font-bold text-slate-400 uppercase tracking-wider pr-3 py-0.5">Revision:</td>
                    <td className="font-semibold text-slate-900 py-0.5">{quote.revision}</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {/* 👥 EXECUTIVE CLIENT & PROJECT CARDS */}
      {(hasClientSection || hasProjectSection) && (
        <div className="grid grid-cols-2 gap-8 mb-8">
          {hasClientSection && (
            <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-blue-800 mb-2 border-b border-slate-200 pb-1">
                Client / Organization
              </h3>
          ...
          {hasProjectSection && (
            <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-blue-800 mb-2 border-b border-blue-200 pb-1">
                Engagement / Project
              </h3>
              ...
              {hasSiteLoc && <p className="text-slate-600 mt-1">Site Location: {quote.siteLocation}</p>}
              {hasBrand && <p className="text-slate-600 mt-1 font-semibold">Specification: {quote.paintBrand}</p>}
              {hasWarranty && (
                <div className="mt-3 inline-flex items-center gap-1.5 bg-blue-900 text-white text-[10px] font-bold px-3 py-1 rounded uppercase tracking-wider">
                  <span>★ {quote.warranty} Year Executive Warranty Included</span>
                </div>
              )}
            </div>
          )}
      ...
      {/* 📊 EXECUTIVE ITEM TABLE */}
      {validSections.length > 0 && (
        <div className="mb-8 space-y-6">
          {validSections.map((sec, secIdx) => (
            <div key={secIdx}>
              {hasVal(sec.title) && (
                <h4 className="font-bold text-blue-950 uppercase tracking-wider text-xs mb-3 pb-1 border-b-2 border-blue-900">
                  {sec.title}
                </h4>
              )}
      ...
      {hasGrandTotal && (
        <div className="flex justify-end mb-8">
          <div className="w-80 rounded-xl border border-slate-300 overflow-hidden shadow-xs bg-slate-50">
            ...
            <div className="p-4 bg-blue-900 text-white flex justify-between items-center font-bold text-sm">
              <span className="uppercase tracking-wider text-xs">Total Net Investment</span>
      ...
      {(hasScope || hasTerms || hasExclusions || hasNotes || hasBankDetails) && (
        <div className="grid grid-cols-2 gap-8 pt-6 border-t border-slate-200 mb-8">
          <div className="space-y-4">
            {hasScope && (
              <div>
                <h4 className="font-bold text-slate-900 uppercase tracking-wider text-[11px] mb-1">Scope of Services</h4>
                  ...
                  <tbody>
                    ...
                    {hasVal(bank.accNo) && (
                      <tr>
                        ...
                        <td className="text-right py-1 font-mono font-bold text-blue-950">{bank.accNo}</td>
                      </tr>
                    )}
                    {hasVal(bank.ifsc) && (
                      <tr>
                        <td className="font-semibold py-1">IFSC / Swift:</td>
      ...
      <div className="pt-8 border-t border-slate-200 flex justify-between items-end">
        <div className="text-[11px] text-slate-500">
          ...
          {hasWebsite && <p className="mt-0.5 text-blue-800">{quote.website}</p>}
        </div>

        {hasSignature && (
          <div className="text-center w-48">
            <div className="h-12 border-b border-slate-300 mb-2 flex items-end justify-center">
              <span className="text-[9px] text-slate-400 uppercase tracking-widest pb-1">[ Executive Signature &amp; Seal ]</span>
            </div>
