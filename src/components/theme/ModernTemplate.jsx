                    ...
                    <tr className="bg-slate-900 text-white font-bold text-[10px] uppercase tracking-wider">
                      ...
                      {sec.components && sec.components.length > 0 ? (
                        sec.components.map((c) => (
                          <th key={c.id} className="py-3 px-4 text-right w-24">{c.name}</th>
                        ))
                      ) : (
                        <>
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                    {sec.items.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3 px-4 text-slate-900 font-semibold whitespace-pre-wrap">{item.desc || item.work}</td>
                        {showUnit && <td className="py-3 px-4 text-center text-slate-500">{item.unit || "—"}</td>}
                        {showQty && <td className="py-3 px-4 text-center font-mono">{item.qty || item.quantity || "1"}</td>}
                        {sec.components && sec.components.length > 0 ? (
                          sec.components.map((c) => {
                            ...
                            return (
                              <td key={c.id} className="py-3 px-4 text-right font-mono">
                                ₹{val.toFixed(2)}
                              </td>
                            );
                          })
                        ...
                        <td className="py-3 px-4 text-right font-mono font-bold text-slate-900">₹{item.total || item.amount}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
